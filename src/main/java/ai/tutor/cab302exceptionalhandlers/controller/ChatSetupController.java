...
public class ChatSetupController {
    @FXML
    private ComboBox<String> responseAttitude;
    @FXML
    private ComboBox<String> educationLevel;
    @FXML
    private ComboBox<String> quizDifficulty;

    @FXML
    private Slider responseLength;
    @FXML
    private TextField chatNameInput;
    @FXML
    private TextField chatTopic;
    @FXML
    ...
    public ChatSetupController(SQLiteConnection db, User authenticatedUser, ChatController mainController,
            String operation, Chat selectedChat) throws SQLException {
    ...
    private void setupStartChatButton() {
        startChatButton.setOnAction(actionEvent -> {
            try {
                mainController.createNewChat(chatNameInput.getText(), responseAttitude.getValue(),
                        quizDifficulty.getValue(), educationLevel.getValue(), chatTopic.getText());
                cancel();
    ...
    private void setupUpdateChatButton() {
        startChatButton.setOnAction(actionEvent -> {
            try {
                mainController.updateChatDetails(mainController.getSelectedChat().getId(), chatNameInput.getText(),
                        responseAttitude.getValue(), quizDifficulty.getValue(), educationLevel.getValue(),
                        chatTopic.getText());
                cancel();
            } catch (SQLException e) {
