...
public class ChatController {
	...
	@FXML
	private Button editChatName;
	@FXML
	private Button addNewChat;
	@FXML
	private Button confirmEditChatName;
	@FXML
	private TextField chatNameField;
	@FXML
	private TextField messageInputField;
	@FXML
	private TextField noChatsField;
	@FXML
	private Button configureChat;
	@FXML
	private TextField welcomeTitle;
	@FXML
	...
	@FXML
	private VBox greetingContainer;
	@FXML
		...
		chatsListView.setCellFactory(listView -> new ListCell<Chat>() {
			...
			private final HBox container = new HBox(selectChat, deleteChatButton);
			{
				...
				Image image = new Image(
						getClass().getResourceAsStream("/ai/tutor/cab302exceptionalhandlers/images/delete.png"));
	...
	private void addQuizMessage(HBox wrapper, HBox horizontalContainer, Label messageLabel, Message message,
	...
	private void addMessage(Message message) {
		...
		ChangeListener<Number> heightListener = new ChangeListener<Number>() {
			@Override
			public void changed(javafx.beans.value.ObservableValue<? extends Number> obs, Number oldHeight,
					Number newHeight) {
	...
	private void handleTakeQuiz(ActionEvent actionEvent, Message message) {
		...
		try {
			FXMLLoader fxmlLoader = new FXMLLoader(QuizWhizApplication.class.getResource("quiz-view.fxml"));
	...
	private void setupActivateEdit() {
		editChatName.setOnAction(actionEvent -> {
			// TODO: Refactor these into class in styles.css and change the css class
			// instead
			editChatName.setVisible(false);
	...
	private void setupCreateChatButton() {
		// TODO: Create chat based on parameters extracted from UI elements and refresh
		// page
	...
	private void loadChatSettings(ActionEvent actionEvent, String operation) {
		try {
			FXMLLoader fxmlLoader = new FXMLLoader(QuizWhizApplication.class.getResource("chat-setup-view.fxml"));

			ChatSetupController controller = new ChatSetupController(db, currentUser, this, operation,
					getSelectedChat());
			fxmlLoader.setController(controller);
	...
	private void setupLogoutButton() {
		logoutButton.setOnAction(actionEvent -> {
			FXMLLoader fxmlLoader = new FXMLLoader(QuizWhizApplication.class.getResource("login-view.fxml"));
	...
	public Chat createNewChat(String name, String responseAttitude, String quizDifficulty, String educationLevel,
			String studyArea) throws IllegalArgumentException, SQLException {
		...
		if (validateNullOrEmpty(educationLevel)) {
			educationLevel = null;
		}
		if (validateNullOrEmpty(studyArea)) {
			studyArea = null;
		}
	...
	public void updateChatDetails(int chatId, String name, String responseAttitude, String quizDifficulty,
			String educationLevel, String studyArea)
	...
	public void updateChatName(int chatId, String newName)
			throws IllegalArgumentException, NoSuchElementException, SQLException {
	...
	// Create a Message object from the AI's response output using a user's Message
	// object as input
	// If AI generation fails, create the Message object with default feedback
	// content
	...
	// Create a QuizQuestion object from the AI's response message if it is a quiz
	// message
	public QuizQuestion createNewQuizQuestion(String questionContent, Quiz quiz)
		...
		// TODO: Depending on AI response quizContent extract number from
		// questionContent or assign dynamically
	...
	// Create an AnswerOption object from the AI's response message if it is a quiz
	// message
	public AnswerOption createNewQuestionAnswerOption(String answerOptionContent, QuizQuestion quizQuestion)
			throws IllegalStateException, IllegalStateException, IllegalArgumentException, SQLException {
		...
		// TODO: Depending on AI response quizContent extract option, value and
		// correctness
		String option = "Option";
		String value = "Answer option statement";
		boolean isAnswer = true;

		if (answerOptionDAO.getQuestionAnswerOption(quizQuestion.getMessageId(), quizQuestion.getNumber(),
				option) != null) {
			throw new IllegalStateException("Answer option already exists");
		}

		AnswerOption answerOption = new AnswerOption(quizQuestion.getMessageId(), quizQuestion.getNumber(), option,
				value, isAnswer);
