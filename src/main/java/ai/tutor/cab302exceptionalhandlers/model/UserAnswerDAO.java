...
public class UserAnswerDAO implements IUserAnswerDAO {
    ...
    private void createTable() throws SQLException {
        try (Statement createTable = connection.createStatement()) {
            createTable.execute("CREATE TABLE IF NOT EXISTS userAnswers (" + "messageId INTEGER,"
                    + "attempt INTEGER NOT NULL CHECK (attempt >= 1),"
                    + "questionNumber INTEGER NOT NULL CHECK (questionNumber >= 1)," + "answerOption VARCHAR NOT NULL,"
